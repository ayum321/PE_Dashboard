...
REM use start-backend\legacy-ui.bat instead.
set "PE_UI_MODE=api"
...
echo   Legacy FastAPI UI: start-backend\legacy-ui.bat
echo  ================================================================
echo   Working dir : %CD%
echo.
...
if not exist "frontend\PE_Dashboard_MFE\source\package-lock.json" (
echo   [ERROR] Missing frontend\PE_Dashboard_MFE\source\package-lock.json.
    pause
    exit /b 1
)

if exist "frontend\PE_Dashboard_MFE\source\node_modules\react-scripts\bin\react-scripts.js" (
call npm --prefix frontend\PE_Dashboard_MFE\source ls --depth=0 >nul 2>&1
    if not errorlevel 1 (
...
call npm --prefix frontend\PE_Dashboard_MFE\source ci --no-audit --no-fund
if errorlevel 1 (
...
set "FILES_OK=1"
for %%F in (backend\PE_Dashboard_API\app\main.py backend\PE_Dashboard_API\app\_find_port.py backend\PE_Dashboard_API\app\_open_browser.py backend\PE_Dashboard_API\app\_seed_config.py backend\PE_Dashboard_API\app\_cleanup_stale.py) do (
    if not exist "%%F" (
...
!PY! backend\PE_Dashboard_API\app\_check_pe_config_refs.py
if errorlevel 1 (
...
!PY! backend\PE_Dashboard_API\app\_seed_config.py >nul 2>&1
...
!PY! backend\PE_Dashboard_API\app\_cleanup_stale.py --quiet 2>nul
...
for /f "usebackq delims=" %%P in (`!PY! backend\PE_Dashboard_API\app\_find_port.py 2^>nul`) do set "PORT=%%P"
...
set "frameUrlPath=/"
call npm --prefix frontend\PE_Dashboard_MFE\source run setLocalEnv >nul 2>&1
if errorlevel 1 (
    echo   [ERROR] React runtime configuration could not be generated.
    pause
    exit /b 1
)
start "PE MFE" cmd /c "set API_BASE_URL=!MFE_API_URL! ^& set appName=PE Audit Dashboard ^& set frameUrlPath=/ ^& cd /d %CD%\frontend\PE_Dashboard_MFE\source ^& npm run start:standalone"
start "PE Browser" /B !PY! backend\PE_Dashboard_API\app\_open_browser.py 127.0.0.1 3000 mfe
!PY! -m uvicorn %APP% --app-dir app --host %HOST% --port !PORT! --reload --reload-dir backend\PE_Dashboard_API\app\routers --reload-dir backend\PE_Dashboard_API\app\services --reload-dir backend\PE_Dashboard_API\app\templates --reload-dir backend\PE_Dashboard_API\app\static

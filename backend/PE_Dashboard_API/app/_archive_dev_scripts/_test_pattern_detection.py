...
def main():
    ...
    # Attribution must carry the canonical interval through the Ctrl-M join
    # and derive duration from start/end. A stale duration_min on the source
    # spike must not produce an implausible display value in the UI.
    attributed = attribute_spikes({"vm-a": {"spikes": {"Percentage CPU": [{
        "start": "2025-01-01T00:00:00Z", "end": "2025-01-01T05:00:00Z",
        "peak_time": "2025-01-01T02:00:00Z", "peak": 99,
        "duration_min": 999, "severity": "critical",
    }]}}}, [])
    assert attributed["rows"][0]["start"] == "2025-01-01T00:00:00Z"
    assert attributed["rows"][0]["end"] == "2025-01-01T05:00:00Z"
    assert attributed["rows"][0]["duration_min"] == 300, attributed["rows"][0]
    print("  [OK] attribution carries spike bounds and derives 300min from a 5h range")

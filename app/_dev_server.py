...
LEGACY_UI_DIR = str(Path(ROOT).parent / "fastapi-dashboard" / "legacy-ui")
os.chdir(ROOT)
...
def _validate_js() -> bool:
    ...
    for name in ("static/app.js", "static/deep_dive.js"):
        path = os.path.join(LEGACY_UI_DIR, name)
...
def main() -> int:
    ...
    print(f"  Auto-reload watches: routers/ services/ fastapi-dashboard/legacy-ui/")
    ...
    # Dual keeps the legacy comparison UI at /legacy while allowing the React
    # MFE launcher to attach to this same local processing API.
    env = {**os.environ, "PE_UI_MODE": "dual"}

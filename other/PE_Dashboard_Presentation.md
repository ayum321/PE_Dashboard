...
- **3 pillars share one number**: Batch CSV + SLA Matrix XLSX + SOW PDF all
  feed `_compute_sla_matrix`, writing ONE table (`resolved_workflow_df`) that
  every batch/findings/report screen reads. SOW's path is indirect — saved
  to a config store first, then read back as a Tier 2 fallback.
- **2 pillars are separate on purpose**: Resource and Benchmark are parsed
  and cached independently, powering their own tabs. The Executive dashboard
  is the one place batch + resource numbers meet — joined client-side.
- **1 "pillar" isn't a pillar**: the Issues Register is typed directly into
  the browser — no file, no parser, no cache. Closing the tab loses it.

**Rule:** every batch/SLA screen reads from `resolved_workflow_df` — no screen
recomputes its own metrics. Resource, Benchmark, and Issues Register are
honestly separate and don't follow that rule.
...
- **Schedule classification** — three independent functions, not aliases:
  `sla_merger.detect_batch_type()` (full classifier for SLA XLSX parsing;
  priority order ADHOC → CYCLIC → CALENDAR/ANNUAL/MONTHLY variants → the
  standard DAILY/WEEKLY/BIWEEKLY/MONTHLY/QUARTERLY keyword loop),
  `pe_utils.detect_batch_type()` (lightweight 4-type classifier for scoring
  individual jobs), and `sla_engine.classify_schedule()` (works on raw
  schedule text). No parity test currently enforces they agree on a name.

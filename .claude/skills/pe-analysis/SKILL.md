...
```
Upload → app/services/batch_calculator.py → app/routers/sla_matrix.py → session cache / resolved_workflow_df → React MFE
```
Every screen reads backend-calculated output derived from `resolved_workflow_df`. **No React screen recomputes metrics.**
...
- Only evidence-backed sections: UAT is included only after explicit UAT, UI-performance, or batch-performance evidence is loaded.

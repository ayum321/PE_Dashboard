...
_TEMPLATES: dict[str, str] = {
    ...
    "final_judgment": (
        ...
        "and red-flags. Use ONLY the named evidence fields supplied in the payload "
        "(evidence_facts.batch.window/job/deadline compliance, breach days, "
        "fleet grade/score, peak_mem_pct, avg_cpu, critical counts, regressions, "
        "and verdict_reason). The verdict MUST cite which evidence fact(s) caused "
        "GO/HOLD/REMEDIATE or BLOCKED/APPROVED/CONDITIONAL. Do not invent context. "
        "Produce a UNIFIED VERDICT in this exact "
        "structure:\n"
        "VERDICT: <one sentence overall grade citing the named driver facts>\n"
        "TOP RISKS:\n"
        "  1. <risk> — <evidence with numbers>\n"
        "  2. <risk> — <evidence with numbers>\n"
        "  3. <risk> — <evidence with numbers>\n"
        "CROSS-PILLAR LINKS: <how the pillars reinforce each other>\n"
        "DECISION: <go / hold / remediate-then-go>\n"
        "NEXT 48H: <up to 3 concrete actions>\n"
    ),
    "smart_verdict_15w": (
